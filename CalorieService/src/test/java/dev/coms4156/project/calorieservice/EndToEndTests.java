...
/**
 * End-to-end tests that exercise the service via HTTP requests against an embedded
 * server, with persistence backed by Google Cloud Firestore. Tests seed
 * and clean data using FirestoreService.
 */
...
public class EndToEndTests {
  ...
  void setup() {
    ...
    assumeTrue(credentialsReady,
        "GOOGLE_APPLICATION_CREDENTIALS not set. Skipping end-to-end tests");
  }
  ...
  void addFoodAndAlternatives() throws Exception {
    String category = "Snack-" + System.currentTimeMillis();
    String target = foodJson(7500, category, 300);
    String alt1 = foodJson(7501, category, 150);
    String alt2 = foodJson(7502, category, 100);
    ...
    org.junit.jupiter.api.Assertions.assertTrue(ids.contains(7501));
    org.junit.jupiter.api.Assertions.assertTrue(ids.contains(7502));
  ...
  void userLikeAndRecommendFlows() throws Exception {
    ...
    int uid3 = 8010;
    firestoreService.addClient(new Client("missinguser", uid3));
    ...
    URI missUser = UriComponentsBuilder.fromUriString(url("/client/recommend"))
